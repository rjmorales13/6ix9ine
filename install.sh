...
if [ -x "$BIN_DIR/t69" ]; then
  # Write a .command file and open it — guaranteed new Terminal window.
  LAUNCHER="$APP_SUPPORT_DIR/launch-t69.command"
  cat > "$LAUNCHER" <<EOF
#!/bin/sh
exec "$BIN_DIR/t69"
EOF
  chmod +x "$LAUNCHER"
  # Save install window, open launcher, close install window.
