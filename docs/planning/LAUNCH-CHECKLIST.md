...
- ⬜ **[CLI]** `release.yml`'s "Update Homebrew formula" step only commits the regenerated `Formula/6ix9ine.rb` to **this repo's** `main` — it does not push to the actual tap Homebrew reads from (`rjmorales13/homebrew-6ix9ine`, a separate repo). Every release so far (v1.0.0, v1.0.1, v1.0.2, v1.0.3) has required a second, manual step: clone `homebrew-6ix9ine`, copy over the updated formula, commit, push. Worth automating (e.g. a second job pushing to the tap repo via a PAT with write access) so a merged release can't ship without the tap being updated to match.
